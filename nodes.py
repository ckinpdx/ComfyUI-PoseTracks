...
class PTAudioFeatureExtractor:
    ...
    RETURN_TYPES = ("PT_AUDIO_FEATURES",)
...
class PTBasePoseGenerator:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {"custom_pose": ("PT_POSE", {"tooltip": "Override with custom pose when pose_type='custom'"})}
        }
    
    RETURN_TYPES = ("PT_POSE",)
...
class PTBeatDetector:
    ...
    def detect(self, audio, frame_count, fps):
        ...
        print(f"[PT] Beats: {len(beat_frames)}, Tempo: {tempo_val:.1f}")
...
class PTBeatDrivenPose:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "base_pose": ("PT_POSE", {"tooltip": "Base skeleton pose from DWPose or Generator"}),
                "beat_info": ("PT_BEAT_INFO", {"tooltip": "Beat detection data from PTBeatDetector"}),
                "audio_features": ("PT_AUDIO_FEATURES", {"tooltip": "Audio analysis from PTAudioFeatureExtractor"}),
    ...
    def generate(self, base_pose, beat_info, audio_features, dance_style, interaction_mode, energy_style, motion_smoothness, anticipation, groove_amount, bass_intensity, treble_intensity, phrase_bars, staging_mode, hit_easing, pose_blend, loop_mode, seed):
        ...
        print(f"[PT] Animating {char_count} characters. Style: {dance_style}, Phrase: {phrase_bars} bars, Easing: {hit_easing}, Loop: {loop_mode}")
...
class PTPoseRenderer:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "pose_sequence": ("PT_POSE_SEQUENCE", {"tooltip": "Animated pose sequence from PTBeatDrivenPose"}),
...
class PTPoseFromDWPose:
    ...
    def extract(self, image_width, image_height, depth, fov, dw_poses=None, pose_keypoint=None):
        data = dw_poses if dw_poses is not None else pose_keypoint
        if data is None: return PTBasePoseGenerator().generate("active_idle", 1, 150.0, 400.0, depth, 0, 0)

        # Unwrap SCAIL-Pose / DWPose outer wrapper: {"poses": [...], "swap_hands": bool}
        ...
        if len(candidates) == 0:
            print("[PT] No candidates found in DWPose data. Fallback to Generator.")
            return PTBasePoseGenerator().generate("active_idle", 1, 150.0, 400.0, depth, 0, 0)
            
        print(f"[PT] Found {len(candidates)} candidates in DWPose data.")
...
class PTAlignPoseToReference:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "pose_sequence": ("PT_POSE_SEQUENCE", {"tooltip": "Animated pose sequence to align"}),
                "reference_pose": ("PT_POSE", {"tooltip": "Reference pose — frame 0 of the output will be exactly this pose"}),
                "transition_frames": ("INT", {"default": 6, "min": 0, "max": 30,
                    "tooltip": "Frames to blend from the reference pose into the sequence. 0 = hard cut after frame 0."}),
            }
        }
    RETURN_TYPES = ("PT_POSE_SEQUENCE",)
    ...
    def align(self, pose_sequence, reference_pose, transition_frames=6):
...
class PTAISTBeatDance:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "beat_info": ("PT_BEAT_INFO", {"tooltip": "Beat detection from PTBeatDetector"}),
                "audio_features": ("PT_AUDIO_FEATURES", {"tooltip": "Audio features for energy mapping"}),
    ...
    def generate(self, library, beat_info, audio_features, sync_dancers,
        ...
        beat_frame_set = set(beat_frames.tolist())
...
class PTAISTFullSequence:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "audio_features": ("PT_AUDIO_FEATURES", {"tooltip": "Audio features for frame count/fps"}),
    ...
    def generate(self, library, audio_features, genre, transition_frames, start_beat, seed, 
        ...
        for frame_idx in range(frame_count):
            # Map to source frame (current_src_frame tracks position within the active sequence)
            ...
            if src_frame_low >= len(current_sequence) - 1:
                if available_sequences:
                    ...
                    # Frame 0 of new sequence is rendered this iteration via explicit override below;
                    # advance current_src_frame past it so next iteration starts at frame 1.
                    current_src_frame = time_scale

                    print(f"[AIST Full] Chaining to: {genre}/{new_fname} ({len(current_sequence)} frames)")

                    # Reset frame indices for current iteration
                    src_frame_low = 0
...
NODE_CLASS_MAPPINGS = {
    ...
    "PTBeatDetector": PTBeatDetector,
    "PTBeatDrivenPose": PTBeatDrivenPose,
    "PTPoseRenderer": PTPoseRenderer,
    "PTPoseFromDWPose": PTPoseFromDWPose,
    "PTAlignPoseToReference": PTAlignPoseToReference,
    "PTAISTLibraryLoader": PTAISTLibraryLoader,
    "PTAISTBeatDance": PTAISTBeatDance,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "PTAudioFeatureExtractor": "PT Audio Features",
    "PTBasePoseGenerator": "PT Base Pose",
    "PTBeatDetector": "PT Beat Detect",
    "PTBeatDrivenPose": "PT Beat-Driven Dance",
    "PTPoseRenderer": "PT Pose Render",
    "PTPoseFromDWPose": "PT Pose from DWPose",
    "PTAlignPoseToReference": "PT Align to Reference",
    "PTAISTLibraryLoader": "PT AIST Library",
    "PTAISTBeatDance": "PT AIST Beat Dance", 
