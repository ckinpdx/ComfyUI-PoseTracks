...
Outputs PT_POSE_SEQUENCE compatible with PT Pose Renderer.
"""
...
class PTMixamoFBX:
    """
    Load FBX animation and convert to PoseTracks pose sequence.
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            ...
            "optional": {
                "pose_ref": ("PT_POSE", {"tooltip": "Reference pose for scaling/positioning"}),
                "pose_sequence_in": ("PT_POSE_SEQUENCE", {"tooltip": "Chain from previous motion"}),
            }
        }
    ...
    RETURN_TYPES = ("PT_POSE_SEQUENCE", "INT", "STRING")
    ...
    def load_fbx(self, fbx_file, blender_version, target_fps, transition_frames,
                 pose_ref=None, pose_sequence_in=None):
        ...
        elif pose_ref is not None:
            input_poses = []
            ref_joints = pose_ref['joints'].numpy()
            char_count = pose_ref.get('char_count', 1)
            last_pose = None
        ...
        elif pose_ref is not None and transition_frames > 0:
            ref_joints_np = pose_ref['joints'].numpy()
...
# Node mappings
NODE_CLASS_MAPPINGS = {
    "PTMixamoFBX": PTMixamoFBX,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "PTMixamoFBX": "PT FBX Motion",
}
